 ...
 *
 */
public class Assignment72 extends Assignment{
	...
	protected void createAssignment() {
		...
		behavior.addCondition("EventA", "[Start] AND NOT [EventB]");
		behavior.addCondition("EventB", "[Start] AND NOT [EventA]");
		behavior.addCondition("TaskA", "[EventA]");
		behavior.addCondition("TaskB", "[EventB]");
		behavior.addCondition("End", "[TaskA] OR [TaskB]");
		addBehavior(behavior);
		
		CaseType caseType1 = new CaseType(60000, 60000, "[End]", 1000000, new String[] {"Start","EventA","End"});
		CaseType caseType2 = new CaseType(60000, 60000, "[End]", 1000000, new String[] {"Start","EventB","End"});
		addCaseType(caseType1);
		addCaseType(caseType2);
	}
